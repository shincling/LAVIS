...
class Blip2Qformer(Blip2Base):
    ...
    PRETRAINED_MODEL_CONFIG_DICT = {
        ...
        "scratch_364": "configs/models/blip2/blip2_pretrain_img364.yaml",
    ...
    def __init__(
        ...
        self.max_txt_len = 120

    def forward(self, samples):
        if "image" in samples and "audio" not in samples:
            image = samples["image"]
            print("Image shape: ", image.shape)
        elif "audio" in samples and "image" not in samples:
            image = samples["audio"]
...
@registry.register_model("blip2_audio")
class Blip2Qformer_audio(Blip2Base):
    """
    BLIP2 first-stage model with Q-former and ViT.
    Supported model types:
        - pretrained: pretrained model
    ...
    def forward(self, samples):
        if "audio" in samples and "image" not in samples:
            audio = samples["audio"] # 
        text = samples["text_input"]
        ...
        query_tokens = self.query_tokens.expand(audio_embeds.shape[0], -1, -1) # bs, 32, 768
        ...
        text_input_ids_world = concat_all_gather(text_tokens.input_ids)
        text_attention_mask_world = concat_all_gather(text_tokens.attention_mask)
        image_embeds_world = all_gather_with_grad(audio_embeds)
        with torch.no_grad():
            weights_t2i = F.softmax(sim_t2i, dim=1) + 1e-4
            weights_t2i[:, rank * bs : rank * bs + bs].fill_diagonal_(0)
            weights_i2t = F.softmax(sim_i2t, dim=1) + 1e-4
        ...
        image_embeds_all = torch.cat(
            [audio_embeds, image_embeds_neg, audio_embeds], dim=0
        )  # pos, neg, pos
        image_atts_all = torch.ones(image_embeds_all.size()[:-1], dtype=torch.long).to(
            audio.device
        )
